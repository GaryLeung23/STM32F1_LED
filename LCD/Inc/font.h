...
//总共：3个字符集（12*6、16*8和24*12），用户可以自行新增其他分辨率的字符集。
//每个字符所占用的字节数为:(size/8+((size%8)?1:0))*(size/2),其中size:是字库生成时的点阵大小(12/16/24...)

//12*6 ASCII字符集点阵 1206占用12个字节，这个12字节是如何体现出了的呢，第一列前八个点占一字节，紧接着一字节高四位占余下4个点（也就是四位），剩余低4位舍弃不用
__attribute__ ((section (".ext_sram"))) const unsigned char asc2_1206[95][12] = {
...
//16*8 ASCII字符集点阵
__attribute__ ((section (".ext_sram")))const unsigned char asc2_1608[95][16] = {
...
//24*12 ASICII字符集点阵
__attribute__ ((section (".ext_sram"))) const unsigned char asc2_2412[95][36] = {
