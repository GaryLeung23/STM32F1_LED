...
/**
  ...
  *     @arg 0 :计算失败
  */
static uint8_t XPT2046_Calculate_CalibrationFactor(_touch_xy *pDisplayCoordinate, _touch_xy *pScreenSample,
                                                   _touch_calibration *pCalibrationFactor) {
    uint8_t ucRet = 1;
...
uint8_t XPT2046_Touch_Calibrate(uint8_t LCD_Mode) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        LCD_ShowString((lcddev.hor_res - (strlen(pStr) * font_size / 2)) >> 1, lcddev.ver_res >> 1, lcddev.hor_res,
                       font_size, font_size, (uint8_t *) pStr);
        lcd_delay_ms(100);                             //适当的延时很有必要
        LCD_DrawCross(strCrossCoordinate[i].x, strCrossCoordinate[i].y);  //显示校正用的“十”字

        while (!XPT2046_ReadAdc_Smooth_XY_qucik(
                &strScreenSample[i]));               //读取XPT2046数据到变量pCoordinate，当ptr为空时表示没有触点被按下
    }
    XPT2046_CS_DISABLE();
    ...
    usTest_x = ((CalibrationFactor.An * strScreenSample[3].x) + (CalibrationFactor.Bn * strScreenSample[3].y) +
                CalibrationFactor.Cn) / CalibrationFactor.Divider;        //取一个点计算X值
    usTest_y = ((CalibrationFactor.Dn * strScreenSample[3].x) + (CalibrationFactor.En * strScreenSample[3].y) +
                CalibrationFactor.Fn) / CalibrationFactor.Divider;    //取一个点计算Y值
    ...
    pStr = "try again";
    //插入空格，居中显示		
    LCD_ShowString((lcddev.hor_res - (strlen(pStr) * font_size / 2)) >> 1, (lcddev.ver_res >> 1) + 50, lcddev.hor_res,
                   font_size,
                   font_size, (uint8_t *) pStr);
...
uint8_t XPT2046_Get_TouchedPoint(_touch_xy *pDisplayCoordinate, _touch_para *pTouchPara) {
    ...
    if (XPT2046_ReadAdc_Smooth_XY_qucik(&strScreenCoordinate)) {
        pDisplayCoordinate->x = ((pTouchPara[lcddev.scan_mode].dX_X * strScreenCoordinate.x) +
                                 (pTouchPara[lcddev.scan_mode].dX_Y * strScreenCoordinate.y) +
                                 pTouchPara[lcddev.scan_mode].dX);
        pDisplayCoordinate->y = ((pTouchPara[lcddev.scan_mode].dY_X * strScreenCoordinate.x) +
                                 (pTouchPara[lcddev.scan_mode].dY_Y * strScreenCoordinate.y) +
                                 pTouchPara[lcddev.scan_mode].dY);
    }
