...
uint8_t XPT2046_Touch_Calibrate(uint8_t LCD_Mode) 
{
    ...
    { 
        ...
        LCD_ShowString((lcddev.hor_res-(strlen(pStr) * font_size /2))>>1, lcddev.ver_res >> 1, lcddev.hor_res, font_size, font_size, (uint8_t*)pStr);
...
Failure:
    ...
    pStr = "try again";
    //插入空格，居中显示		
    LCD_ShowString((lcddev.hor_res-(strlen(pStr) * font_size /2))>>1, (lcddev.ver_res >> 1) + 50, lcddev.hor_res, font_size,
