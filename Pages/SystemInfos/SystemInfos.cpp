...
void SystemInfos::onViewWillAppear() {
    View.Group_Init();//add objs to group

    Model.SetStatusBarStyle(DataProc::STATUS_BAR_STYLE_BLACK);

    timer = lv_timer_create(onTimerUpdate, 1000, this);//1s
    lv_timer_ready(timer);

    View.SetScrollToY(_root, -LV_VER_RES, LV_ANIM_OFF);//先将屏幕滑到-LV_VER_RES，然后因为focus的效果，最终效果为从下面滑动上来
