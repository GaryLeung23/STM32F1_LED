...
void StartupView::Create(lv_obj_t* root)
{
    ...
    lv_obj_set_style_border_side(cont, LV_BORDER_SIDE_BOTTOM, 0);//只有下边框
    lv_obj_set_style_border_width(cont, 3, 0);
    lv_obj_set_style_border_post(cont, true, 0);//设置是在绘制子项之前还是之后绘制边框。true: 子类之后, false: 子类之前
    lv_obj_center(cont);
    ui.cont = cont;
    ...
    ui.anim_timeline = lv_anim_timeline_create();//create timeline
...
#define ANIM_DEF(start_time, obj, attr, start, end) \
    ...
    lv_anim_timeline_add_wrapper(ui.anim_timeline, wrapper);//添加到timeline
}
