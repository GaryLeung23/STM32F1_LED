...
//瓦片地图地图 图片路径 dirPath/level/tileX/tileY.extName
...
//加载系统配置文件 设置瓦片地图系统的参数
static bool onLoad(Account* account)
{
    //add to storageService from file。storageService里的node，它的value是使用指针指向对应的值。所以LoadFile会直接覆盖默认值
    bool success = storageService.LoadFile();
    if (!success)
    {
        LV_LOG_WARN("Load " CONFIG_SYSTEM_SAVE_FILE_PATH " error");
        success =  storageService.LoadFile(CONFIG_SYSTEM_SAVE_FILE_BACKUP_PATH);
        if (!success)
        {
            LV_LOG_WARN("Load " CONFIG_SYSTEM_SAVE_FILE_BACKUP_PATH " error");
        }
    }
...
static void onNotify(Account* account, Storage_Info_t* info)
{
    ...
    case STORAGE_CMD_SAVE:
        ...
        break;
    //添加数据Node到storageService的NodePool中
    case STORAGE_CMD_ADD:
        ...
        break;
    //从storageService的NodePool中删除数据Node
...
static int onEvent(Account* account, Account::EventParam_t* param)
{
    if (param->event == Account::EVENT_SUB_PULL)
    {
        ...
        info->totalSizeMB = SD_GetCardSizeMB();
...
static void onSDEvent(bool insert)
{
    if(insert)
    {
        ...
        //使用MainAccount发送Load命令 重新读取json file
...
//    HAL::SD_SetEventCallback(onSDEvent); //这是检测SD card insert的callback函数
}
