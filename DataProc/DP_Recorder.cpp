...
//向gpx文件写入<trkpt>的信息 包括接经纬度 高度 时间
static void Recorder_RecPoint(Recorder_t* recorder, HAL::GPS_Info_t* gpsInfo)
{
    ...
    recorder->gpx.setEle(formatDobleValue(gpsInfo->altitude, 2));
    recorder->gpx.setTime(timeBuf);

    String gpxStr = recorder->gpx.getPt(
                        GPX_TRKPT,
                        formatDobleValue(gpsInfo->longitude, 6),
                        formatDobleValue(gpsInfo->latitude, 6)
                    );
...
static void Recorder_RecStart(Recorder_t* recorder, uint16_t time)
{
    ...
    if (res == LV_FS_RES_OK)
    {
        ...
        gpx->setMetaName(RECORDER_GPX_META_NAME); //metadata标签下的
        gpx->setMetaDesc(RECORDER_GPX_META_DESC);
        gpx->setName(filepath);//trk标签下的
        gpx->setDesc("");

        Recorder_FileWriteString(file_p, gpx->getOpen().c_str());//<gpx>
        Recorder_FileWriteString(file_p, gpx->getMetaData().c_str());//<metadata>
        Recorder_FileWriteString(file_p, gpx->getTrakOpen().c_str());//<trk>
        Recorder_FileWriteString(file_p, gpx->getInfo().c_str());
        Recorder_FileWriteString(file_p, gpx->getTrakSegOpen().c_str());//<trkseg>
...
static void Recorder_RecStop(Recorder_t* recorder)
{
    ...
    Recorder_FileWriteString(file_p, gpx->getTrakSegClose().c_str());//</trkseg>
    Recorder_FileWriteString(file_p, gpx->getTrakClose().c_str());//</trk>
    Recorder_FileWriteString(file_p, gpx->getClose().c_str());//</gpx>
    lv_fs_close(file_p);
...
static int onNotify(Recorder_t* recorder, Recorder_Info_t* info)
{
    ...
    //notify TrackFilter account ，根据cmd处理命令
...
static int onEvent(Account* account, Account::EventParam_t* param)
{
    ...
    //其他account来 pull 数据Recorder_Info_t
    ...
    //其他account来 Notify 命令 (开始 暂停 恢复 停止等)
